# Sloth — Database Schema

> **Version:** 1.0.0  
> **Engine:** SQLite via sqlx (async)  
> **Location:** `~/.local/share/sloth-tui/sloth.db`  
...
Sloth uses a single SQLite database file for all persistent local state. The schema is designed for:
- Fast lookups by content ID (indexed)
- Resume timestamp accuracy (float precision)
- Offline-first — all data is readable without network
- Migration-safe — schema versioned, applied with `PRAGMA user_version`
...
```sql
-- Get resume position for a specific episode
SELECT resume_position, duration, completed
FROM watch_history
WHERE media_id = ?1 AND season = ?2 AND episode = ?3;
```

### Recently Watched (Home Screen)

```sql
-- Last 20 watched items with media metadata
SELECT 
    m.id, m.title, m.kind, m.poster_url, m.rating,
    wh.season, wh.episode, wh.episode_title,
    wh.resume_position, wh.duration, wh.watched_at
...
### Continue Watching (Resume Incomplete)

```sql
-- Items where progress is between 5% and 85% (worth resuming)
SELECT 
    m.id, m.title, m.kind,
    wh.season, wh.episode, wh.resume_position, wh.duration
FROM watch_history wh
JOIN media m ON m.id = wh.media_id
WHERE wh.completed = 0
  AND wh.resume_position > 0
  AND (wh.resume_position / wh.duration) BETWEEN 0.05 AND 0.85
ORDER BY wh.watched_at DESC
LIMIT 10;
```

### AniList Dirty Sync Queue

```sql
-- Entries that need pushing to AniList
SELECT anilist_id, status, progress, score, notes
FROM anilist_entries
WHERE dirty = 1;
```

### Upcoming F1 Sessions

```sql
-- Next 5 F1 sessions across all session types
WITH sessions AS (
    SELECT name, circuit, country,
           fp1_time AS ts, 'FP1' AS session FROM f1_calendar WHERE fp1_time > unixepoch()
    UNION ALL
    SELECT name, circuit, country,
           fp2_time, 'FP2' FROM f1_calendar WHERE fp2_time > unixepoch()
    UNION ALL
    SELECT name, circuit, country,
           qualifying_time, 'Qualifying' FROM f1_calendar WHERE qualifying_time > unixepoch()
    UNION ALL
    SELECT name, circuit, country,
           race_time, 'Race' FROM f1_calendar WHERE race_time > unixepoch()
)
SELECT * FROM sessions ORDER BY ts LIMIT 5;
```
...
Migrations are embedded in the binary using `sqlx::migrate!`:
...
```
migrations/
├── 0001_initial.sql     # All tables above
...
| Table | Index | Purpose |
|---|---|---|
| `media` | `idx_media_kind` | Filter by content type |
| `media` | `idx_media_title` | Title search |
| `media` | `idx_media_tmdb` | TMDB ID lookup |
| `media` | `idx_media_anilist` | AniList ID lookup |
| `watch_history` | `idx_history_media` | All episodes for a show |
| `watch_history` | `idx_history_watched` | Recent history sort |
| `watch_history` | `idx_history_completed` | Filter incomplete |
| `f1_calendar` | `idx_f1_season` | Season filter |
| `f1_calendar` | `idx_f1_race` | Chronological sort |
| `sports_events` | `idx_sports_sport` | Sport type filter |
| `sports_events` | `idx_sports_time` | Scheduled order |
| `sports_events` | `idx_sports_live` | Live-first ordering |
...
| Table | Retention |
|---|---|
| `media` | Forever (space is cheap, content IDs stable) |
| `watch_history` | Forever (user's watch record) |
| `favorites` | Forever (user explicit) |
| `anilist_entries` | Forever (synced from AniList) |
| `trakt_entries` | Forever |
| `sports_events` | Auto-purge rows older than 48h (background task) |
| `notifications_sent` | Auto-purge rows older than 7 days |
| `f1_calendar` | Overwritten each season |

The sports_events purge runs at app startup:

```sql
DELETE FROM sports_events WHERE fetched_at < (unixepoch() - 172800);
DELETE FROM notifications_sent WHERE sent_at < (unixepoch() - 604800);
```
