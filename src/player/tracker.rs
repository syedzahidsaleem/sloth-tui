//! MPV and external player state tracking scripts and persistence paths.
...
options.read_options(opts, "sloth")
...
/// Ensures the MPV lua tracking script is written to disk and returns its path.
pub fn ensure_tracker_script() -> Option<PathBuf> {
    ...
    let path = dir.join("sloth_tracker.lua");
    if !path.exists()
...
/// Computes the filesystem path for an item's playback state JSON file.
