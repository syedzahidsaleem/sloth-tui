...
/// Active runtime video player backend wrapping the specific player instance.
pub enum PlayerBackend {
    Mpv(MpvPlayer),
    Vlc(VlcPlayer),
    Iina(MpvPlayer), // IINA supports mpv IPC natively
}

impl PlayerBackend {
    /// Waits for the active player process to exit, returning (position, duration).
    pub async fn wait_for_exit(&mut self) -> (Option<f64>, Option<f64>) {
        match self {
            Self::Mpv(p) => {
                let pos = p.wait_for_exit().await;
                (pos, None) // mpv gives position but not duration via our IPC
            }
            Self::Vlc(p) => p.wait_for_exit().await,
            Self::Iina(p) => {
                let pos = p.wait_for_exit().await;
                (pos, None)
            }
...
/// Launches player according to config, tracks resume position via IPC/HTTP, and saves state to SQLite upon exit.
pub async fn launch_player(
    ...
    let mut player_backend = match backend_choice {
        crate::config::PlayerBackend::Vlc => {
            PlayerBackend::Vlc(vlc::VlcPlayer::spawn(stream, resume_pos, config).await?)
        }
        crate::config::PlayerBackend::Iina => {
    ...
    let completed = if let (Some(p), Some(d)) = (pos, dur) {
        d > 0.0 && (p / d >= 0.85)
    } else {
        false
    };

    let resume_position = pos.unwrap_or(0.0);
...
/// Environment variable override for primary video player.
pub const ENV_SLOTH_PLAYER: &str = "SLOTH_PLAYER";
/// Legacy environment variable override for video player.
pub const ENV_MOVIEBOX_PLAYER: &str = "MOVIEBOX_PLAYER";

const TERMUX_PREFIX_USR: &str = "/data/data/com.termux/files/usr";
...
/// Standard UNIX binary lookup paths for players.
...
pub enum PlayerKind {
    /// MPV player (cross-platform).
    Mpv,
    /// IINA player (macOS native).
    Iina,
    /// VLC media player (cross-platform).
    Vlc,
    /// Android Intent opener (Termux / Android).
    AndroidIntent,
}

impl PlayerKind {
    /// Returns human-readable label for player.
    ...
    /// Returns config key representation.
    ...
    /// Parses string name into PlayerKind.
...
/// Detects available video players on the current operating system.
...
/// Checks whether a player backend supports custom HTTP headers.
...
/// Returns list of players capable of setting custom HTTP headers.
...
/// Constructs process command to launch video playback.
pub fn command(
    kind: PlayerKind,
    url: &str,
    subtitle: Option<&str>,
...
/// Supported Android intent invocation tools.
#[allow(clippy::enum_variant_names)]
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum AndroidOpener {
    /// Termux Activity Manager (termux-am).
    TermuxAm(String),
    /// Termux file opener (termux-open).
    TermuxOpen(String),
    /// Termux URL opener (termux-open-url).
    TermuxOpenUrl(String),
    /// Rooted Android system Activity Manager.
    #[cfg(target_os = "android")]
    SystemAm(String),
}

/// Probes available Android opener binaries on the system.
pub fn probe_android_openers() -> Vec<AndroidOpener> {
    let mut openers = Vec::new();

    if let Some(custom) = configured_executable("SLOTH_ANDROID_PLAYER_PATH") {
    ...
    let termux_am_static = format!("{TERMUX_PREFIX_USR}/bin/termux-am");
    if Path::new(&termux_am_static).is_file() {
        push_unique(AndroidOpener::TermuxAm(termux_am_static));
    }
    let termux_open_static = format!("{TERMUX_PREFIX_USR}/bin/termux-open");
    if Path::new(&termux_open_static).is_file() {
        push_unique(AndroidOpener::TermuxOpen(termux_open_static));
    }
    let termux_open_url_static = format!("{TERMUX_PREFIX_USR}/bin/termux-open-url");
...
/// Returns detected Android openers.
...
/// Builds an Android opener command for a specific launcher.
...
/// Returns ordered list of opener commands to attempt.
...
fn probe_iina_resolution() -> Option<IinaResolution> {
    if let Some(executable) = configured_executable("SLOTH_IINA_PATH") {
...
/// Returns whether IINA is configured as application fallback on macOS.
#[cfg(target_os = "macos")]
pub fn iina_is_app_fallback() -> bool {
    matches!(iina_resolution(), Some(IinaResolution::AppFallback))
}

/// Returns whether IINA is configured as application fallback (always false on non-macOS).
#[cfg(not(target_os = "macos"))]
pub fn iina_is_app_fallback() -> bool {
    false
}
...
/// Returns prioritized list of candidate filesystem paths for MPV on Windows.
...
/// Returns prioritized list of candidate filesystem paths for VLC on Windows.
...
fn configured_executable(variable: &str) -> Option<String> {
    let val = std::env::var(variable)
        .or_else(|_| {
            if let Some(rest) = variable.strip_prefix("SLOTH_") {
                std::env::var(format!("MOVIEBOX_{rest}"))
            } else {
...
/// Formats command-line script options for MPV tracking.
pub fn format_mpv_script_opts(
    ...
    format!(
        "sloth-provider={provider},sloth-subject_id={subject_id},sloth-season={season},sloth-episode={episode},sloth-state_file={state_file_str},moviebox-provider={provider},moviebox-subject_id={subject_id},moviebox-season={season},moviebox-episode={episode},moviebox-state_file={state_file_str}"
    )
}
...
mod tests {
    ...
    fn vlc_command_preserves_supported_playback_options() {
        let command = vlc_command(
            ...
            &[
                ...
                ("User-Agent".into(), "Sloth-Test".into()),
        ...
        assert!(args.contains(&"--http-user-agent=Sloth-Test".into()));
    ...
    fn vlc_command_normalizes_windows_subtitle_paths() {
        let command = vlc_command(
            ...
            Some(r"C:\Users\User\AppData\Local\Sloth-Tui\subs\sub.srt"),
            &[],
            None,
            None,
        );
