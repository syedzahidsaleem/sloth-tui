//! Home screen, search input, continue watching deck, and TV channel list views.
...
pub fn landing_split(
    ...
    let compact_logo = tier.is_compact() || (tv_mode && area.width < 50) || area.width < 46;
...
pub fn draw(frame: &mut Frame, area: Rect, state: &mut AppState, theme: &Theme) {
    ...
    match state.active_tab {
        Tab::Anime => {
            ...
            crate::tui::screens::anime::render(frame, content_area, state, theme, is_editing);
        ...
        Tab::Sports => {
            crate::tui::screens::sports::render(frame, content_area, &state.sports_tab, theme);
            if state.show_settings_popup {
                crate::tui::widgets::settings::draw(frame, area, state, theme);
            }
            return;
