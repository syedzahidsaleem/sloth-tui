//! Terminal capability detection, window title management, and image support checks.

fn env(name: &str) -> String {
    std::env::var(name).unwrap_or_default()
}

/// Checks whether basic 16-color ASCII UI should be used.
...
/// Determines if the current terminal environment can render inline graphics/images.
pub fn should_query_images() -> bool {
    if std::env::var("SLOTH_NO_IMAGE")
    ...
    if is_inside_tmux()
        && std::env::var("SLOTH_IMAGE_PROTOCOL").is_err()
...
/// Checks whether the terminal has a light background.
...
/// Sets the terminal window title bar text.
...
mod tests {
    ...
    fn test_should_query_images_guards() {
        unsafe {
            std::env::set_var("SLOTH_NO_IMAGE", "1");
            assert!(!should_query_images());
            std::env::remove_var("SLOTH_NO_IMAGE");

            std::env::set_var("SLOTH_IMAGE_PROTOCOL", "none");
            assert!(!should_query_images());
            std::env::remove_var("SLOTH_IMAGE_PROTOCOL");
            ...
            std::env::set_var("SLOTH_IMAGE_PROTOCOL", "kitty");
            assert!(should_query_images());
            std::env::remove_var("SLOTH_IMAGE_PROTOCOL");
