...
/// Generates the x-client-token header value from current timestamp.
...
/// Builds the canonical signature payload required by the MovieBox gateway.
...
/// Generates HMAC-MD5 x-tr-signature required for MovieBox API calls.
...
/// Constructs complete set of signed HTTP request headers for MovieBox endpoints.
...
pub(crate) fn generate_client_info_and_ua() -> (String, String) {
    ...
    let android = android_versions[fastrand::usize(0..android_versions.len())];
    let device = redmi_devices[fastrand::usize(0..redmi_devices.len())];
    let version_code = version_codes[fastrand::usize(0..version_codes.len())];
    let network = network_types[fastrand::usize(0..network_types.len())];
    let timezone = timezones[fastrand::usize(0..timezones.len())];
...
pub(crate) fn random_spoofed_ip() -> String {
    ...
    let c: u8 = fastrand::u8(1..254);
    let d: u8 = fastrand::u8(1..254);
    format!("{}.{}.{}", prefix, c, d)
}
