//! MovieBox streaming provider module.
...
/// Standard Referer header used when resolving MovieBox streams.
...
impl Provider for client::MovieBoxClient {
    ...
    async fn search(&self, query: &str, _kind: MediaType) -> Result<Vec<Media>, ProviderError> {
        let json = self.search(query, 1).await.map_err(ProviderError::from)?;
        let items = adapt::moviebox_search_json_to_catalog(&json);
        Ok(items.into_iter().map(Media::from).collect())
    }
...
impl client::MovieBoxClient {
    ...
    /// Retrieves detailed media information for an item.
...
impl MovieBoxClient {
    /// Retrieves stream playback information from MovieBox BFF API.
    ...
    /// Performs search query against MovieBox BFF search API.
    ...
    /// Search query auto-suggestion endpoint.
    pub async fn suggest(&self, query: &str) -> Result<Value, ScraperError> {
        self.search(query, 1).await
    }

    /// Fetches full metadata details and season listings for a media subject.
    ...
    /// Retrieves operating tab content for homepage browse feed.
    ...
    /// Queries resources list with optional resolution filter.
    ...
    /// Fetches a page of resources with pagination metadata.
    ...
    /// Queries all available video resolutions for a subject collection.
    ...
    /// Queries external subtitles for a resource ID.
...
/// Constructs resource page query path.
