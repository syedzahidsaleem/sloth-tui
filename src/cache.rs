//! Caching system: typed MsgPack binary cache, atomic durable file writes, and LRU memory cache.
...
/// Magic 4-byte header identifying typed msgpack binary cache files.
pub const CACHE_MAGIC: [u8; 4] = *b"MBC1";

/// Cache envelope storing payload along with expiration epoch timestamp.
...
/// Retrieves and deserializes typed data from a binary msgpack or legacy JSON cache file.
...
/// Serializes and writes data wrapped in a CacheEnvelope using MsgPack format atomically.
...
/// Computes lowercase hex MD5 hash of input string.
...
/// Atomically and durably writes bytes to a file by writing to a temporary file then renaming.
...
/// Asynchronously and durably writes bytes to a file using temporary file and atomic replace.
...
/// Returns root cache subfolder for a given provider.
...
/// Returns disk path for cached stream releases.
...
/// Loads cached stream releases if present and not expired.
...
/// Saves stream releases to cache with intelligent CloudFront TTL parsing.
...
/// Invalidates cached stream records for a specific item.
...
/// Returns disk path for cached media details.
...
/// Loads cached media details from disk.
...
/// Saves media details to disk cache.
...
/// Removes cached media details for an item.
pub fn invalidate_provider_details_cache(provider: ProviderKind, subject_id: &str) {
    let path = get_provider_details_path(provider, subject_id);
    let _ = fs::remove_file(path);
}

/// Returns disk path for search result caching.
...
/// Returns disk path for homepage catalog cache.
...
/// Loads cached homepage catalog and browse metrics.
...
/// Saves homepage catalog and browse metrics to cache.
...
/// Returns cache path for an addon catalog.
...
/// Loads cached addon catalog items.
...
/// Saves addon catalog items to cache.
...
/// Returns cache path for an addon manifest.
...
/// Loads cached image bytes for a namespace and ID.
...
/// Writes image bytes to namespaced cache.
...
/// Returns disk path for cached subtitles/captions.
...
/// Saves subtitle tracks to disk cache.
...
/// Recursively removes all files and directories inside `dir` while keeping `dir` intact.
...
/// Clears all application caches across disks and temp folders.
pub fn clear_all_cache() -> Result<(), String> {
    ...
    let temp_subs = std::env::temp_dir()
...
/// Launches background task to clean cache files older than 7 days.
